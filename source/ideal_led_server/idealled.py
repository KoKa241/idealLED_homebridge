...
class IDEALLEDInstance:
    def __init__(self, address, delay: int, fw_version: str) -> None:
        self.loop = asyncio.get_running_loop()
        self._mac = address
        self._delay = delay
        self._device: BLEDevice | None = None
        # Device lookup will be done asynchronously when needed
    ...
    def _notification_handler(
        ...
        LOGGER.debug(
            f"BLE Notification decrypted: {self.name}: {bytearray_to_hex_format(clear_data)}"
        )
        # self.local_callback()
    ...
    async def set_rgb_color(
        self, hs_col: Tuple[int, int], brightness: int | None = None
    ):
        ...
        if brightness is None:
            LOGGER.debug(
                "Brightness not specified, using current value: %s", self._brightness
            )
        ...
        red   = int(red) # Removed >> 3 bit shift to allow full 8-bit brightness
        green = int(green)
        blue  = int(blue)
    ...
    async def update(self):
        ...
        try:
            await self._ensure_connected()
            # We cannot read state currently, so we just ensure connection
            # and leave self._is_on as its current value instead of forcing False.
    ...
    async def _ensure_connected(self) -> None:
        ...
        async with self._connect_lock:
            ...
            if not self._device:
                ...
                self._device = await BleakScanner.find_device_by_address(self._mac, timeout=10.0)
                if not self._device:
                    raise Exception(
